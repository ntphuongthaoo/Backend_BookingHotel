...
class REVIEW_CONTROLLER {
  async addReview(req, res) {
    try {
      ...
      const review = await REVIEW_SERVICE.addReview(userId, bookingId, roomId, rating, comment);
  ...
  async getReviewsByRoomId(req, res) {
    try {
      ...
      const reviews = await REVIEW_SERVICE.getReviewsByRoomId(roomId);

      if (!reviews || reviews.length === 0) {
        return res.status(404).json({ success: false, msg: "Không có đánh giá nào cho phòng này." });
      }
  ...
  async updateReview(req, res) {
    try {
      ...
      const updatedReview = await REVIEW_SERVICE.updateReview(reviewId, userId, rating, comment);

      return res.json({ success: true, msg: "Chỉnh sửa đánh giá thành công", review: updatedReview });
    } catch (error) {
